...
void WavInFile::init()
{
    ...
    */
    m_dataStartPosInFile = (int)ftell(fptr);
    dataRead = 0;
    position = 0;
}
...
int WavInFile::read(short *buffer, long startSampleIndex, long endSampleIndex) {
    ...
    case 8:
    {
        ...
        fseek(fptr, m_dataStartPosInFile, SEEK_SET);
...
int WavInFile::read(short *buffer, long startTime, long endTime, int maxElems) {
    ...
    case 8:
    {
        ...
        int numBytes = (endIndexInByte - startIndexInByte + 1);
        if (numBytes > maxElems) numBytes = maxElems;
        fseek(fptr, startIndexInByte, SEEK_SET);
        // 8 bit format
        unsigned char *temp = (unsigned char*)getConvBuffer(maxElems);
        int i;
        numBytes = (int)fread(temp, 1, numBytes, fptr);
    ...
    case 16:
    {
        ...
        int numBytes = (endIndexInShort - startIndexInShort + 1) * 2;
        if (numBytes > maxElems * 2) numBytes = maxElems * 2;
        fseek(fptr, startIndexInShort * 2, SEEK_SET);
...
int WavInFile::read(float *buffer, long startTime, long endTime, int maxElems) {
    ...
    int numBytes = (endIndexInShort - startIndexInShort + 1) * bytesPerSample;
    if (numBytes > maxElems * bytesPerSample) numBytes = maxElems * bytesPerSample;
    fseek(fptr, startIndexInShort * bytesPerSample, SEEK_SET);
...
int WavInFile::read(float **buffer, int num_samples) {
    long afterDataRead=0;
    int numBytes=0;
    int numElems=0;
    int bytesPerSample=0;

    int numChannels = header.format.channel_number;
    int maxElems = num_samples * numChannels;
    ...
    case 1:
    {
        ...
        if (numChannels == 1) {
            for (int i = 0; i < numElems; i ++)
            {
                buffer[0][i] = (float)(temp2[i] * conv - 1.0);
            }
        } else {
            for (int i = 0, j = 0; i < numElems; i+=2, j++)
            {
                buffer[0][j] = (float)(temp2[i] * conv - 1.0);
                buffer[1][j] = (float)(temp2[i+1] * conv - 1.0);
            }
        }
        break;
    }
    ...
    case 2:
    {
        ...
        if (numChannels == 1) {
            ...
            {
                short value = temp2[i];
                buffer[0][i] = (float)(_swap16(value) * conv);
            }
        } else {
            for (int i = 0, j = 0; i < numElems; i += 2, j++)
            {
                short value = temp2[i];
                buffer[0][j] = (float)(_swap16(value) * conv);
                value = temp2[i+1];
                buffer[1][j] = (float)(_swap16(value) * conv);
            }
        }
        break;
    }
    ...
    case 3:
    {
        ...
        if (numChannels == 1) {
            ...
            for (int i = 0, j = 0; i < numElems; i +=2, j++)
            {
                ...
                value = _swap32(value) & 0x00ffffff;             // take 24 bits
                value |= (value & 0x00800000) ? 0xff000000 : 0;  // extend minus sign bits
                buffer[0][j] = (float)(value * conv);
                temp2 += 3;

                value = *((int*)temp2);
                value = _swap32(value) & 0x00ffffff;             // take 24 bits
                value |= (value & 0x00800000) ? 0xff000000 : 0;  // extend minus sign bits
                buffer[1][j] = (float)(value * conv);
                temp2 += 3;
            }
        }
        break;
    ...
    case 4:
    {
        ...
        if (numChannels == 1) {
            ...
            {
                int value = temp2[i];
                buffer[0][i] = (float)(_swap32(value) * conv);
            }
        } else {
            for (int i = 0, j = 0; i < numElems; i +=2, j++)
            {
                int value = temp2[i];
                buffer[0][j] = (float)(_swap32(value) * conv);
                value = temp2[i+1];
                buffer[1][j] = (float)(_swap32(value) * conv);
            }
        }
        break;
    }
...
int WavInFile::readHeaderBlock()
{
    ...
    if (strcmp(label, fmtStr) == 0)
    {
        ...
        if (nDump > 0)
        {
            ...
            // printf("nLen to be actually read:%d\n",nLen);
        }
        ...
        if (nDump > 0)
        {
            // printf("skip the extra data nDump:%d\n",nDump);
            // fseek(fptr, nDump, SEEK_CUR);
            char tmp[nDump];
            fread(tmp, nDump, 1, fptr); // dummy read
        }

        return 0;
    }
    else if (strcmp(label, infoStr) == 0)
    {
...
std::string WavInFile::getFileName() const {
    return name;
}
...
int WavInFile::getWavFormat() const
{
    return header.format.fixed;
}
...
#include <vector>
std::vector<string> g_wavfileNameVec;
std::vector<WavOutFile*> g_WavOutFileObjVector;
void WavOutFile::EndWriteAllArray(){
#ifdef DEBUG
    g_wavfileNameVec.clear();
    for (int i=0; i<g_WavOutFileObjVector.size(); i++) {
        if(g_WavOutFileObjVector[i]){
            delete g_WavOutFileObjVector[i];
        }
    }
    g_WavOutFileObjVector.clear();
#endif
}
void WavOutFile::EndWriteArray(const char* fpath){
    #ifdef DEBUG
    int idx=-1;
    bool find=false;
    for (int i=0; i<g_wavfileNameVec.size(); i++) {
        if(g_wavfileNameVec[i].compare(fpath)==0){
            find=true;
            idx = i;
            break;
        }
    ...
    if (find && idx!=-1) {
        delete g_WavOutFileObjVector[idx];
        std::vector<string>::iterator sit = g_wavfileNameVec.begin()+idx;
        std::vector<WavOutFile*>::iterator wit = g_WavOutFileObjVector.begin()+idx;
        g_wavfileNameVec.erase(sit);
        g_WavOutFileObjVector.erase(wit);
    }
    #endif
}

void WavOutFile::write(float *const * buffer, int num_samples)
{
    int numBytes=0;
    int bytesPerSample=0;
    int numChannels = header.format.channel_number;

    int numElems = num_samples * numChannels;
    ...
    case 1:
    {
        ...
        if (numChannels == 1) {
            for (int i = 0; i < numElems; i ++)
            {
                temp2[i] = (unsigned char)saturate(buffer[0][i] * 128.0f + 128.0f, 0.0f, 255.0f);
            }
        } else {
            for (int i = 0, j = 0; i < numElems; i +=2, j++)
            {
                temp2[i] = (unsigned char)saturate(buffer[0][j] * 128.0f + 128.0f, 0.0f, 255.0f);
                temp2[i+1] = (unsigned char)saturate(buffer[1][j] * 128.0f + 128.0f, 0.0f, 255.0f);
            }
        }
        break;
    }
    ...
    case 2:
    {
        ...
        if (numChannels == 1) {
            for (int i = 0; i < numElems; i ++)
            {
                short value = (short)saturate(buffer[0][i] * 32768.0f, -32768.0f, 32767.0f);
                temp2[i] = _swap16(value);
            }
        } else {
            for (int i = 0, j = 0; i < numElems; i +=2, j++)
            {
                short value = (short)saturate(buffer[0][j] * 32768.0f, -32768.0f, 32767.0f);
                temp2[i] = _swap16(value);
                value = (short)saturate(buffer[1][j] * 32768.0f, -32768.0f, 32767.0f);
                temp2[i+1] = _swap16(value);
            }
        }
        break;
    }
    ...
    case 3:
    {
        ...
        if (numChannels == 1) {
            for (int i = 0; i < numElems; i ++)
            {
                int value = saturate(buffer[0][i] * 8388608.0f, -8388608.0f, 8388607.0f);
                *((int*)temp2) = _swap32(value);
                temp2 += 3;
            }
        } else {
            for (int i = 0, j = 0; i < numElems; i +=2, j++)
            {
                int value = saturate(buffer[0][j] * 8388608.0f, -8388608.0f, 8388607.0f);
                *((int*)temp2) = _swap32(value);
                temp2 += 3;
                value = saturate(buffer[1][j] * 8388608.0f, -8388608.0f, 8388607.0f);
                *((int*)temp2) = _swap32(value);
                temp2 += 3;
            }
        }
    ...
    case 4:
    {
        ...
        if (numChannels == 1) {
            for (int i = 0; i < numElems; i ++)
            {
                int value = saturate(buffer[0][i] * 2147483648.0f, -2147483648.0f, 2147483647.0f);
                temp2[i] = _swap32(value);
            }
        } else {
            for (int i = 0, j = 0; i < numElems; i +=2, j++)
            {
                int value = saturate(buffer[0][j] * 2147483648.0f, -2147483648.0f, 2147483647.0f);
                temp2[i] = _swap32(value);
                value = saturate(buffer[1][j] * 2147483648.0f, -2147483648.0f, 2147483647.0f);
                temp2[i+1] = _swap32(value);
            }
        }
        break;
    }
...
int audioRead(std::string &inputPath, int samprate,
        int nchan, std::vector<float> &input){

    WavInFile *vocalFile = new WavInFile(inputPath.c_str());
    if(vocalFile == NULL){
        return -1;
    }
    int datalen = vocalFile->getDataSizeInBytes() / 2;
    input.resize(datalen);
    vocalFile->read(input.data(), datalen);
//    samplerate = vocalFile->getSampleRate();
//    numChannels = vocalFile->getNumChannels();
    delete vocalFile;
    return 0;
}

int audioWrite(std::string &outputPath, int samprate,
        int nchan, std::vector<float> &output){
    WavOutFile *outFileWriter = new WavOutFile(
            outputPath.c_str(), samprate, 16, nchan
    );
    if(outFileWriter == NULL){
        return -1;
    }
    outFileWriter->write(output.data(), output.size());
    delete outFileWriter;
    return 0;
}
