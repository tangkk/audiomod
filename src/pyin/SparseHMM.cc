...
const vector<float>
SparseHMM::calculateObsProb(const vector<pair<float, float> > data)
{
...
SparseHMM::decodeViterbi(std::vector<vector<float> > obsProb,
                         vector<float> *scale) 
{
    if (obsProb.size() < 1) {
        return vector<int>();
    ...
    // declaring variables
    std::vector<float> delta = std::vector<float>(nState);
    std::vector<float> oldDelta = std::vector<float>(nState);
    ...
    for (size_t iState = 0; iState < nState; ++iState)
    {
        float currentValue = oldDelta[iState];
