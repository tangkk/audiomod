...
const vector<float>
MonoNoteHMM::calculateObsProb(const vector<pair<float, float> > pitchProb)
{
    ...
    vector<float> out = vector<float>(par.n);
    float tempProbSum = 0;
    for (size_t i = 0; i < par.n; ++i)
    {
        if (i % par.nSPP != 2)
        {
            ...
            if (nCandidate > 0)
            {
                ...
                for (size_t iCandidate = 0; iCandidate < nCandidate; ++iCandidate)
                {
                    float currDist = std::abs(getMidiPitch(i)-pitchProb[iCandidate].first);
                    if (currDist < minDist)
                    {
                        minDist = currDist;
...
MonoNoteHMM::build()
{
    ...
    for (size_t iPitch = 0; iPitch < (par.nS * par.nPPS); ++iPitch)
    {
        ...
        float mu = par.minPitch + iPitch * 1.0/par.nPPS;
    ...
    for (size_t iPitch = 0; iPitch < (par.nS * par.nPPS); ++iPitch)
    {
        ...
        float probSumSilent = 0;

        vector<float> tempTransProbSilent;
        for (size_t jPitch = 0; jPitch < (par.nS * par.nPPS); ++jPitch)
        {
                 ...
                 && semitoneDistance < par.maxJump))
            {
                ...
                float tempWeightSilent = boost::math::pdf(noteDistanceDistr, 
                                                           semitoneDistance);
