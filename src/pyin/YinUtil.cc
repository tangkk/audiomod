...
void 
YinUtil::slowDifference(const float *in, float *yinBuffer, const size_t yinBufferSize) 
{
    yinBuffer[0] = 0;
    float delta ;
    int startPoint = 0;
...
void 
YinUtil::fastDifference(const float *in, float *yinBuffer, const size_t yinBufferSize) 
{
    ...
    float *audioTransformedReal = new float[frameSize];
    float *audioTransformedImag = new float[frameSize];
    float *nullImag = new float[frameSize];
    float *kernel = new float[frameSize];
    float *kernelTransformedReal = new float[frameSize];
    float *kernelTransformedImag = new float[frameSize];
    float *yinStyleACFReal = new float[frameSize];
    float *yinStyleACFImag = new float[frameSize];
...
void 
YinUtil::fastDifference_fixed(const float *in, float *yinBuffer, const size_t yinBufferSize) 
{
    // DECLARE AND INITIALISE
    // initialisation of most of the arrays here was done in a separate function,
    // with all the arrays as members of the class... moved them back here.
    ...
    size_t frameSize = 2 * yinBufferSize;
	SRFFT* m_fft = new SRFFT(frameSize);
	float *powerTerms = new float[yinBufferSize];
	int *s32in = new int[frameSize];
	COMPLEX *audioTransformed = new COMPLEX[frameSize];
	COMPLEX *kernelTransformed = new COMPLEX[frameSize];
	COMPLEX *yinStyleACF = new COMPLEX[frameSize];
    ...
    for (size_t j = 0; j < frameSize; ++j)
    {
		s32in[j] = in[j]*16384.0;
		audioTransformed[j].real = s32in[j];
		audioTransformed[j].imag = 0;
		kernelTransformed[j].real = 0;
		kernelTransformed[j].imag = 0;
		yinStyleACF[j].real = 0;
		yinStyleACF[j].imag = 0;
    }
    ...
    for (size_t j = 0; j < frameSize; ++j)
    {
		yinStyleACFReal[j] = 0.0;
		yinStyleACFImag[j] = 0.0;
		audioTransformedReal[j] = 0.;
        audioTransformedImag[j] = 0.;
    }

	 // 1. data, // 2. half of the data, disguised as a convolution kernel
	for (size_t j = 0; j < yinBufferSize; ++j) {
        //kernel[j] = in[yinBufferSize-1-j];
		kernelTransformed[j].real = s32in[yinBufferSize-1-j];
    }

	m_fft->Split_radix(audioTransformed, kernelTransformed);

	// 3. convolution via complex multiplication -- written into
    for (size_t j = 0; j < frameSize; ++j) {
		yinStyleACFReal[j] = (float)audioTransformed[j].real*kernelTransformed[j].real - (float)audioTransformed[j].imag*kernelTransformed[j].imag; // real
		yinStyleACFImag[j] = (float)audioTransformed[j].real*kernelTransformed[j].imag + (float)audioTransformed[j].imag*kernelTransformed[j].real; // imaginary
		yinStyleACFReal[j] /= 16384.0*16384.0;
		yinStyleACFImag[j] /= 16384.0*16384.0;
    }
    Vamp::FFT::inverse(frameSize, yinStyleACFReal, yinStyleACFImag, audioTransformedReal, audioTransformedImag);
	...
	// 3. convolution via complex multiplication -- written into
    for (size_t j = 0; j < frameSize; ++j) {
		float tmp1 = (float)audioTransformed[j].real*kernelTransformed[j].real - (float)audioTransformed[j].imag*kernelTransformed[j].imag; // real
		float tmp2 = (float)audioTransformed[j].real*kernelTransformed[j].imag + (float)audioTransformed[j].imag*kernelTransformed[j].real; // imaginary
		yinStyleACF[j].real = tmp1 / 16384;
		yinStyleACF[j].imag = tmp2 / 16384;
    }
	m_fft->invert_FFT(yinStyleACF);
    
    // CALCULATION OF difference function
    // ... according to (7) in the Yin paper.
    for (size_t j = 0; j < yinBufferSize; ++j) {
        // taking only the real part
		yinBuffer[j] = powerTerms[0] + powerTerms[j] - 2 * yinStyleACF[j+yinBufferSize-1].real / 16384.0;
    }
#endif

    delete [] powerTerms;
	delete [] s32in;
	delete [] audioTransformed;
	delete [] kernelTransformed;
	delete [] yinStyleACF;
...
void 
YinUtil::cumulativeDifference(float *yinBuffer, const size_t yinBufferSize)
{    
    size_t tau;
...
int 
YinUtil::absoluteThreshold(const float *yinBuffer, const size_t yinBufferSize, const float thresh)
{
    size_t tau;
    size_t minTau = 0;
    float minVal = 1000.;
...
std::vector<float>
YinUtil::yinProb(const float *yinBuffer, const size_t prior, const size_t yinBufferSize, const size_t minTau0, const size_t maxTau0) 
{
    size_t minTau = 2;
    ...
    std::vector<float> peakProb = std::vector<float>(yinBufferSize);
    ...
    // float factor = 1.0 / (0.25 * (nThresholdInt+1) * (nThresholdInt + 1)); // factor to scale down triangular weight
    size_t minInd = 0;
    float minVal = 42.f;
    ...
    while (tau+1 < maxTau)
    {
        if (yinBuffer[tau] < thresholds[thresholds.size()-1] && yinBuffer[tau+1] < yinBuffer[tau])
        {
            ...
            while ((currThreshInd > -1) && (thresholds[currThreshInd] > yinBuffer[tau])) {
				...
				//if (thresholds[currThreshInd] <= yinBuffer[tau]) break;
                peakProb[tau] += distribution[currThreshInd];
                currThreshInd--;
            }
    ...
    if (peakProb[minInd] > 1) {
        ...
        return(std::vector<float>(yinBufferSize));
    }
...
float
YinUtil::parabolicInterpolation(const float *yinBuffer, const size_t tau, const size_t yinBufferSize) 
{
...
float 
YinUtil::sumSquare(const float *in, const size_t start, const size_t end)
{
    float out = 0;
