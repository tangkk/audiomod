...
YinVamp::FeatureSet
YinVamp::process(const float *inputBuffers, RealTime timestamp)	//const float *const *inputBuffers
{
    //timestamp = timestamp + Vamp::RealTime::frame2RealTime(m_blockSize/2, (int)(m_inputSampleRate));//lrintf(m_inputSampleRate)
    FeatureSet fs;
    ...
    } else
    {
        if (fabs(yo.f0) < m_fmax && fabs(yo.f0) > m_fmin) {
            f.values.push_back(yo.f0);
			//std::cerr << "yo.f0: " << yo.f0 << std::endl;
			//std::cerr << "f.values: " << f.values[0] << std::endl;
            fs[m_outNoF0].push_back(f);
			//std::cerr << "fs[m_outNoF0]: " << fs[0][0].values[0] << std::endl;
        }
    }
