...
m_freqs(0)
{
    ...
    m_nPitch = 69 * m_nBPS;
    m_freqs = vector<float>(2*m_nPitch);
...
const vector<float>
MonoPitchHMM::calculateObsProb(const vector<pair<float, float> > pitchProb)
{
    vector<float> out = vector<float>(2*m_nPitch+1);
    float probYinPitched = 0;
    // BIN THE PITCHES
    for (size_t iPair = 0; iPair < pitchProb.size(); ++iPair)
    {
        float freq = 440. * std::pow(2, (pitchProb[iPair].first - 69)/12);
    ...
    float probReallyPitched = m_yinTrust * probYinPitched;
...
{
    // INITIAL VECTOR
    init = vector<float>(2*m_nPitch, 1.0 / 2*m_nPitch);
