...
Yin::Yin(size_t frameSize, size_t inputSampleRate, float thresh, bool fast) : 
...
Yin::YinOutput
Yin::process(const float *in) const {
    
    float* yinBuffer = new float[m_yinBufferSize];
    ...
    float rms = std::sqrt(YinUtil::sumSquare(in, 0, m_yinBufferSize)/m_yinBufferSize);
...
Yin::YinOutput
Yin::processProbabilisticYin(const float *in) const {
    
    float* yinBuffer = new float[m_yinBufferSize];

    // calculate aperiodicity function for all periods
#define PYIN_FIXED_POINT_ENABLE	0
#if (0 == PYIN_FIXED_POINT_ENABLE)
    if (m_fast) YinUtil::fastDifference(in, yinBuffer, m_yinBufferSize); // orig code
#else
	if (m_fast) YinUtil::fastDifference_fixed(in, yinBuffer, m_yinBufferSize);
#endif
    else YinUtil::slowDifference(in, yinBuffer, m_yinBufferSize);

    YinUtil::cumulativeDifference(yinBuffer, m_yinBufferSize);

    vector<float> peakProbability = YinUtil::yinProb(yinBuffer, m_threshDistr, m_yinBufferSize);
    ...
    for (size_t iBuf = 0; iBuf < m_yinBufferSize; ++iBuf)
    {
        ...
        if (peakProbability[iBuf] > 0)
        {
            ...
            yo.freqProb.push_back(pair<float, float>(currentF0, peakProbability[iBuf]));
        }
    }
...
int
Yin::setThreshold(float parameter)
{
