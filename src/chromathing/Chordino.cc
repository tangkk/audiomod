...
Chordino::FeatureSet
Chordino::process(const float *realOut, const float *imagOut, Vamp::RealTime timestamp)
{
...
Chordino::getRemainingFeatures()
{
    ...
    **/
    ...
    snprintf(buffer0, 50, "estimated tuning: %0.1f Hz", cumulativetuning);
...
// dummies
Chordino::FeatureSet
Chordino::getRemainingFeatures0(std::vector<int>) {
    Chordino::FeatureSet fsnull;
    return fsnull;
}

// dummies
Chordino::FeatureSet
Chordino::process(const float *inputBuffers, Vamp::RealTime timestamp) {
    Chordino::FeatureSet fsnull;
    return fsnull;
}