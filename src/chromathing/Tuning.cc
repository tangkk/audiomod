...
Tuning::FeatureSet
Tuning::process(const float *realOut, const float *imagOut, Vamp::RealTime timestamp)
{   
    if (debug_on) cerr << "--> process" << endl;

    // NNLSBase::baseProcess(inputBuffers, timestamp);
    NNLSBase::baseProcess(realOut, imagOut, timestamp);
    ...
    // std::cerr << "m_localTuning.size():" << m_localTuning.size() << std::endl;
    // std::cerr << "localtuning:" << tuning440 << std::endl;
...
Tuning::getRemainingFeatures()
{
    ...
    **/
    ...
    std::cerr << "cumulativetuning:" << cumulativetuning << std::endl;
...
// dummies
Tuning::FeatureSet
Tuning::getRemainingFeatures0(std::vector<int>) {
    Tuning::FeatureSet fsnull;
    return fsnull;
}

Tuning::FeatureSet
Tuning::process(const float *inputBuffers, Vamp::RealTime timestamp) {
    Tuning::FeatureSet fsnull;
    return fsnull;
}