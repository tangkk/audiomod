...
void ConstantQ::sparsekernel()
{
    ...
    float* windowRe = new float [ m_FFTLength ];
    float* windowIm = new float [ m_FFTLength ];
    float* transfWindowRe = new float [ m_FFTLength ];
    float* transfWindowIm = new float [ m_FFTLength ];
    ...
    for (int j = m_uK - 1; j >= 0; --j) {
        ...
        int windowLength = (int)ceil
            (m_dQ * m_FS / (m_FMin * pow(2, (float)j / (float)m_BPO)));

        int origin = m_FFTLength/2 - windowLength/2;

        for (int i = 0; i < windowLength; ++i) {
            float angle = (2.0 * M_PI * m_dQ * i) / windowLength;
            windowRe[origin + i] = cos(angle);
            windowIm[origin + i] = sin(angle);
        }

        // Shape with hamming window
        Window<float> hamming(HammingWindow, windowLength);
        ...
        for (int i = 0; i < m_FFTLength; i++) {
            ...
            float mag = squaredModule(transfWindowRe[i], transfWindowIm[i]);
...
float* ConstantQ::process( const float* fftdata )
{
    ...
    const float *real = &(sk->real[0]);
    const float *imag = &(sk->imag[0]);
    ...
    for (int i = 0; i < sparseCells; i++) {
        ...
        if (col == 0) continue;
        const float & r1 = real[i];
        const float & i1 = imag[i];
        const float & r2 = fftdata[ (2*m_FFTLength) - 2*col - 2 ];
        const float & i2 = fftdata[ (2*m_FFTLength) - 2*col - 2 + 1 ];
...
void ConstantQ::process(const float *FFTRe, const float* FFTIm,
                        float *CQRe, float *CQIm)
{
    ...
    for (int i = 0; i<sparseCells; i++) {
        ...
        const float & r1 = real[i];
        const float & i1 = imag[i];
        const float & r2 = FFTRe[ m_FFTLength - col ];
        const float & i2 = FFTIm[ m_FFTLength - col ];
        // add the multiplication
