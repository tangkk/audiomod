...
public:
    ...
    FeatureSet process(const float *realOut, const float *imagOut, Vamp::RealTime timestamp);
    FeatureSet getRemainingFeatures();

    // old dummies here
    FeatureSet getRemainingFeatures0(std::vector<int>);
    FeatureSet process(const float *inputBuffers, Vamp::RealTime timestamp);
