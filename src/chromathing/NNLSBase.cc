...
void
NNLSBase::baseProcess(const float *realOut, const float *imagOut, Vamp::RealTime timestamp)
{   
    m_frameCount++;   
    float *magnitude = new float[m_blockSize/2];
	
    // const float *fbuf = inputBuffers[0]; // this should be an FFT spectrum, storing real and imag of FFT interleaved?
    float energysum = 0;
    // make magnitude
    float maxmag = -10000;
    m_rollon = 1.0;
    // cerr << "m_blockSize:" << m_blockSize << endl;
    for (int iBin = 0; iBin < static_cast<int>(m_blockSize/2); iBin++) {
        // cerr << iBin << ":" << realOut[iBin] << "," << imagOut[iBin] << endl;
        // magnitude[iBin] = sqrt(fbuf[2 * iBin] * fbuf[2 * iBin] + fbuf[2 * iBin + 1] * fbuf[2 * iBin + 1]);
        magnitude[iBin] = sqrt(realOut[iBin] * realOut[iBin] + imagOut[iBin] * imagOut[iBin]);
    ...
    if (maxmag < 2) {
        cerr << "timestamp " << timestamp << ": " << maxmag << " very low magnitude, setting magnitude to all zeros" << endl;
        for (int iBin = 0; iBin < static_cast<int>(m_blockSize/2); iBin++) {
            magnitude[iBin] = 0;
        }
    } else {
        // cerr << "timestamp " << timestamp << ": " << maxmag << " good frame" << endl;
    }
