...
// Chords profile
static float MajProfile[kBinsPerOctave] = {
...
static float MinProfile[kBinsPerOctave] = { 
    ...
    m_keyStrengths(0)
{
    ...
    chromaConfig.FS = config.sampleRate / (float)m_decimationFactor;
...
//    std::cerr << "chroma frame size = " << m_ChromaFrameSize << ", decimation factor = " << m_DecimationFactor << " therefore block size = " << getBlockSize() << std::endl;
    ...
    m_decimatedBuffer = new float[m_chromaFrameSize];
    m_chromaBuffer = new float[kBinsPerOctave * m_chromaBufferSize];

    memset(m_chromaBuffer, 0,
           sizeof(float) * kBinsPerOctave * m_chromaBufferSize);
    
    m_meanHPCP = new float[kBinsPerOctave];
    
    m_majCorr = new float[kBinsPerOctave];
    m_minCorr = new float[kBinsPerOctave];
    
    m_majProfileNorm = new float[kBinsPerOctave];
    m_minProfileNorm = new float[kBinsPerOctave];

    float mMaj = MathUtilities::mean( MajProfile, kBinsPerOctave );
    float mMin = MathUtilities::mean( MinProfile, kBinsPerOctave );
...
float GetKeyMode::krumCorr( const float *pDataNorm, const float *pProfileNorm, 
...
int GetKeyMode::process(float *pcmData)
{
    int key;
    int j, k;
    ...
    for (k = 0; k < kBinsPerOctave; k++) {
        ...
        m_meanHPCP[k] = mnVal / (float)m_chromaBufferFilling;
    }

    // Normalize for zero average
    float mHPCP = MathUtilities::mean(m_meanHPCP, kBinsPerOctave);
    for (k = 0; k < kBinsPerOctave; k++) {
        m_meanHPCP[k] -= mHPCP;
    }
    ...
    int midpoint = (int)ceil((float)sortlength / 2);
...
float* GetKeyMode::getKeyStrengths() {
    int k;
