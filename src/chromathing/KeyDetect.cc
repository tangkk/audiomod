...
static std::map<int, int> numberToPosition = {
    {7, 1}, {12, 2}, {5, 3}, {10, 4}, {3, 5}, {8, 6}, {1, 7}, {6, 8}, {11, 9}, {4, 10},
    {9, 11}, {2, 12}, {16, 13}, {21, 14}, {14, 15}, {19, 16}, {24, 17}, {17, 18}, {22, 19},
    {15, 20}, {20, 21}, {13, 22}, {18, 23}, {23, 24}
};
...
KeyDetector::initialise(size_t channels, size_t stepSize, size_t blockSize)
{
    ...
    // if (channels > 2) { // support stereo too, but only take the left
        std::cerr << "num channels invalid!" << std::endl;
        return false;
    }
    ...
    std::cerr << "m_stepSize:" << m_stepSize << std::endl;
    std::cerr << "m_blockSize:" << m_blockSize << std::endl;
                     ...
                     Vamp::RealTime now)
{
    ...
    for ( unsigned int i = 0 ; i < m_blockSize; i++ ) {
        // printf("%d\n", i);
        m_inputFrame[i] = (float)inputBuffers[0][i]; // take left only
    }

    int key = m_getKeyMode->process(m_inputFrame);
    // printf("%d\n", key);

    // perform keystrengths first
    Feature ksf;
    ksf.hasTimestamp = false;
    // ksf.values.reserve(25);
    ksf.values.reserve(24);

    Feature tsf;
    tsf.hasTimestamp = false;
    tsf.values.reserve(12);

    float *keystrengths = m_getKeyMode->getKeyStrengths();

    for (int i = 0; i < 24; ++i) {

        // if (i == 12) ksf.values.push_back(-1); // why push_back(-1) here?
    ...
    if (m_first || (key != m_prevKey)) {
        ...
        feature.values.push_back(ksf.values[numberToPosition[key]-1]);
        for (const auto &x : ksf.values) {
            std::cerr << x << ',';
        }
        std::cerr << std::endl;
...
// dummies
KeyDetector::FeatureSet
KeyDetector::getRemainingFeatures0(std::vector<int>) {
    KeyDetector::FeatureSet fsnull;
    return fsnull;
}

// dummies
KeyDetector::FeatureSet
KeyDetector::process(const float *inputBuffers, Vamp::RealTime timestamp) {
    KeyDetector::FeatureSet fsnull;
    return fsnull;
}